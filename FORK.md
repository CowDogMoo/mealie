...
rebase cost. It is `git diff --name-only <base>` restricted to paths that already existed at the
base — where the base is the commit this fork's diff is actually against (`14e66291`), not the
`v3.24.0` tag PLAN.md §7 still names. Everything else the fork adds is a new file — new models,
schemas, repositories, route modules, Vue components, tests, the alembic revision, the image
workflow — and a new file never conflicts. Keep it that way. If the fork ever starts editing a
twenty-fourth upstream file, add it here in the same commit, or the next upstream bump becomes a
surprise.
